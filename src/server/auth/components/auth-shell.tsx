...
export function AuthShell({ children, tone = "default" }: AuthShellProps) {
    const accent =
        tone === "destructive"
            ? "radial-gradient(circle_at_25%_20%,hsl(var(--destructive)/0.08),transparent_60%)"
            : "radial-gradient(circle_at_30%_20%,hsl(var(--primary)/0.08),transparent_60%)";

    return (
        <div
            className="relative flex min-h-dvh items-center justify-center overflow-hidden"
            style={{
                backgroundImage: `${accent}, radial-gradient(circle_at_70%_80%,hsl(var(--muted-foreground)/0.08),transparent_55%)`,
            }}
        >
