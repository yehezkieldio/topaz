...
- A second `magicLinkVerify` call with the *same* token redirects to `.../library?error=INVALID_TOKEN` with no cookie -- confirming the token is consumed atomically on first use, so a screenshotted or reused QR code doesn't grant a second session.
