...
- It's SQLite-file-compatible and supports FTS5 (verify the trigram tokenizer specifically once switched -- this session verified trigram FTS5 against Bun's _bundled_ SQLite build specifically, not libsql's).
...
2. **FTS5 external-content DELETE must run before the content row changes, not after.** `indexTermFts`'s delete-then-insert pattern, called _after_ `UPDATE taxonomy_term SET name = ...`, corrupted the FTS5 shadow index (`SQLITE_CORRUPT_VTAB`, "database disk image is malformed") -- because FTS5's DELETE reads the content table's _current_ row to compute which trigrams to remove, and by the time it ran, the content row already had the _new_ name. Same error occurs deleting a rowid that was never indexed at all (the create path). Fixed by splitting into `insertTermFts` (create path, insert only) and `removeTermFromFts` (rename path, called _before_ the content update, with `insertTermFts` called again after).
...
- FTS5 search finding the term under its _new_ name after rename, confirming the index actually updated.
...
The sync _algorithm_ -- oplog, HLC ordering, apply, pairing -- is real and correct, verified against actual execution, not just typechecked. The blocker was entirely in getting a Next.js server (dev or the compiled binary) to actually serve it over HTTP, which is the `bun:sqlite` issue documented above.
...
## "Connect a phone": thin-client mobile support

Mobile support turned out not to need a mobile-specific sync engine at all. There's no way to run a background server on Android/iOS the way the compiled Bun binary runs on a laptop, so a phone was made a thin client instead: it never gets a local database or joins the pairing mesh, it's just a browser pointed at a laptop's server over Tailscale. The only real ergonomics gap that left was "type a Tailscale hostname and a password on a phone keyboard," which a QR code closes: `/sync` now has a "Connect a phone" card whose QR encodes a one-time magic-link sign-in (better-auth's `magicLink` plugin) for the device's single admin account, addressed at the device's own tailnet hostname/port (the same address embedded in pairing codes) rather than `BETTER_AUTH_URL` (typically `localhost`, meaningless to another device). Scanning it lands the phone's browser in an authenticated `/library`, no typing required.

The plugin only hands the raw token to its `sendMagicLink` callback (the API response is just `{ status: true }`, since better-auth assumes you're emailing the link) -- this app never emails anything, so `src/lib/auth.ts` arms a one-shot module-level resolver (`captureNextMagicLink`) the callback fires into, letting the server action that generates the QR get the token back synchronously.

Verified directly against `auth.api.signInMagicLink`/`auth.api.magicLinkVerify` (not just typechecked), since headless-browser interactivity turned out to be broken in this sandboxed test environment for reasons unrelated to this feature -- see the note below:

- `signInMagicLink({ email: "admin@example.com", callbackURL: "/library" })` succeeds and the capture resolves with a real token.
- The first `magicLinkVerify` call with that token throws a redirect (`statusCode: 302`, `location: .../library`) carrying a real `set-cookie: better-auth.session_token=...` -- over actual HTTP (a phone's browser), this is exactly a normal 302 response with the session cookie attached, which is what `auth.api.*` throws in lieu of writing to a response stream when called outside a real HTTP request.
- A second `magicLinkVerify` call with the _same_ token redirects to `.../library?error=INVALID_TOKEN` with no cookie -- confirming the token is consumed atomically on first use, so a screenshotted or reused QR code doesn't grant a second session.
- The URL built by the server action resolves to `http://<SYNC_TAILNET_HOSTNAME>:<SYNC_PORT>/api/auth/magic-link/verify?...`, confirmed against the actual `env` values rather than assumed.

### An unrelated, pre-existing bug this surfaced: browser click/effect handlers don't fire in this sandbox

While verifying the above through an actual browser (Playwright + the pre-installed Chromium), _no_ client-side interactivity worked at all: clicking the existing sign-in button (`auth-action-form.tsx`, unchanged by this feature) never fired its `handleSubmit`, and `MobileConnectCard`'s own `useEffect` never ran either (checked via `document.querySelector('button').__reactProps$...` being `undefined` on every element, on pages with no code from this session). This affects every interactive component in the app identically, old and new, so it reads as an environment-level issue (e.g. the sandbox's WebSocket-proxying interfering with something Turbopack dev's client depends on to finish attaching) rather than a bug in this feature's code. Confirmed the feature's actual logic is correct by testing `auth.api.signInMagicLink`/`magicLinkVerify` directly (above) instead of through a simulated click. Worth investigating separately if browser-driven verification is needed for other work in this sandbox.
